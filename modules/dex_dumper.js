/**
 * 脚本名称：dex_dumper.js
 * 功能描述：自动提取内存中的DEX文件，支持脱壳分析和DEX动态加载监控
 * 
 * 适用场景：
 *   - 脱壳分析加固保护的Android应用
 *   - 提取动态加载的DEX文件
 *   - 分析应用运行时的类加载行为
 *   - 获取内存中解密后的DEX文件
 *   - 支持常见加固保护如梆梆、爱加密、360加固、腾讯乐固等
 *   - 辅助逆向分析混淆或保护的应用代码
 *
 * 使用方法：
 *   1. 可通过frida_master.js主入口文件加载(推荐)
 *   2. 也可单独使用: frida -U -f 目标应用包名 -l dex_dumper.js --no-pause
 *   3. 或者 frida -U --attach-pid 目标进程PID -l dex_dumper.js
 *   4. 提取的DEX文件将自动保存到/sdcard/frida_dumps/目录
 *
 * 启动方式说明：
 *   - -U: 使用USB连接的设备
 *   - -f: 指定以spawn方式启动的应用包名
 ...
 * 工作原理：
 *   1. ClassLoader监控：
 *      - 监控BaseDexClassLoader、DexClassLoader和InMemoryDexClassLoader的创建
 *      - 拦截类加载过程并从ClassLoader中提取DEX文件
 *
 *   2. 内存扫描：
 *      - 定期扫描应用内存中的Magic Number("dex\n035\0"等DEX文件特征)
 *      - 自动验证和提取完整的DEX文件
 *      - 使用智能算法避免重复提取和处理无效DEX
 *
 *   3. 特定加固绕过：
 *      - 针对不同加固厂商实现特定的脱壳方案
 *      - 支持20多种主流加固方案，包括梆梆、爱加密、360加固等
 *      - 自动优化脱壳参数以适应不同类型的应用
 *
 *   4. 类批量加载：
 *      - 可选择性地强制加载所有类以触发解密
 ...
 *   - 支持多个DEX文件的提取和保存
 *   - 智能去重和完整性验证
 *   - 适配各种Android版本(5.0-13)
 *   - 针对Unity、虚幻引擎等特殊应用的优化
 *
 * 注意事项：
 *   - 需要存储权限才能保存提取的DEX文件
 *   - 在高度定制的加固方案上可能需要手动调整参数
 *   - 在较旧设备上内存扫描可能会影响应用性能
 *   - 最好在应用启动阶段使用以捕获所有动态加载的DEX
 *   - 可通过setOutputDirectory等方法自定义配置
 */
