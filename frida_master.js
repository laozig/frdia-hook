/*
 * 脚本名称：frida_master.js
 * 功能描述：Frida全功能Hook框架主入口，提供全面的逆向分析和安全测试能力
 * 
 * 适用场景：
 *   - 全面分析Android应用的行为和安全机制
 *   - 自动化提取应用加密密钥和算法信息
 *   - 绕过应用各类保护和检测机制
 *   - 监控应用的网络通信、文件操作和敏感API调用
 *   - 收集应用运行时的安全相关信息
 *   - 进行全面的黑盒和灰盒安全测试
 *
 * 使用方法：
 *   1. 修改配置参数section(如需要)，调整日志级别、文件日志等选项
 *   2. frida -U -f 目标应用包名 -l frida_master.js --no-pause
 *   3. 或者 frida -U --attach-pid 目标进程PID -l frida_master.js
 *   4. 框架将自动加载并初始化各个功能模块
 *
 * 启动方式说明：
 *   - -U: 使用USB连接的设备
 ...
 *
 * 主要功能和模块：
 *   - 加密API监控：跟踪和提取所有Java加密API调用和密钥
 *   - 反检测保护：自动绕过常见的反调试、反注入和环境检测机制
 *   - 网络监控：拦截和记录HTTP/HTTPS请求与响应
 *   - 安全API监控：记录应用对敏感API的调用
 *   - 文件操作监控：跟踪应用的文件读写操作
 *   - Native层监控：跟踪JNI函数调用和Native库加载
 *   - 日志系统：灵活配置的日志记录功能
 *
 * 注意事项：
 *   - 默认启用兼容模式，支持Frida 14.0.0及以上版本
 *   - 可以通过修改配置参数调整功能和日志级别
 *   - 在高度加固的应用中可能需要配合其他技术使用
 *   - 大型应用可能产生大量日志，可酌情调整logLevel
 *   - 建议使用spawn模式以便捕获应用启动阶段的行为
 */
